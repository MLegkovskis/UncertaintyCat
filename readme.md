...
- Twenty-four hash-checked reference models available inside authenticated workspaces.
...
- Study-scoped distribution fitting, a deterministic Calibration Studio, explicit Morris-derived model versions, promoted PCE/GPR surrogates, and reliability guidance.
...
Authenticated users land on a single project index. Creating a project opens its overview; models, direct analyses, dimensionality reduction, calibration, surrogate construction, distribution fitting, and previous runs then remain visibly scoped to that project. A new model combines the 24 reference models and the editable Python model into one authoring surface; the model name starts blank, and choosing a reference loads both its name and source unless the name was already edited manually. The Guided Builder compiles its named formulas and distributions into an OpenTURNS [`SymbolicFunction`](https://openturns.github.io/openturns/latest/user_manual/_generated/openturns.SymbolicFunction.html).
...
The threshold decision is versioned numerical metadata, not an AI decision. The selected AI provider gives a separate concise explanation from validated metadata. Validation status, deterministic facts, the AI brief, and the recommended route share one Model Understanding panel. Direct analysis intentionally excludes calibration, Morris, PCE, and GPR: those methods live in dedicated project studios. The direct composer includes bounded target-domain HSIC controls for asking which inputs are associated with a scalar output entering an explicit critical region; reports distinguish that screen from event probability and causal influence. Calibration Studio treats selected continuous model inputs as unknown constant parameters, conditions on named observed explanatory inputs and one observed output, and uses bounded OpenTURNS nonlinear least squares. A reduced model or promoted surrogate then offers two explicit, provenance-preserving handoffs: start another analysis in the current project, or copy the model (and surrogate artifact when applicable) into a new project.
