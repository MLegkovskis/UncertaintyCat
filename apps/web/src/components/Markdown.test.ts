...
describe("report evidence citations", () => {
  it("replaces internal evidence paths with human-readable source labels", () => {
    const formatted = formatEvidenceCitations(
      "Input a leads [analysis.fact:sobol.most_influential_input] with 0.61 【analysis.metric:taylor.first_order_variance】; dependence is [analysis.metric:ancova.sum_correlation_contributions]. Calibration used [analysis.metric:calibration_nlls.rmse_after].",
    );
    ...
    expect(formatted).toContain("Source: Nonlinear Least-Squares Calibration · Rmse After");
