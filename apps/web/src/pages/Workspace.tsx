...
function ModelUnderstandingPane({
  ...
  return (
    <aside className="understanding-pane" aria-label="Model Understanding">
      ...
      <section className="validation-facts">
        <div className="validation-success">
          ...
          <div>
            ...
            <span>{model.metadata.input_dimension} inputs → {model.metadata.output_dimension} outputs · OpenTURNS {model.metadata.openturns_version}</span>
          </div>
        </div>
      ...
      >
        <h3>Model brief <small>generated by Workers AI</small></h3>
...
function WorkflowAssessment({ model, projectId, compact = false }: { model: ModelVersion; projectId: string; compact?: boolean }) {
  ...
  return (
    <section className={`workflow-assessment ${path} ${compact ? "compact" : ""}`} aria-label="Recommended analysis route">
      ...
      {content.href.startsWith("/") ? <Link className="button primary" to={content.href}>{content.action} <ArrowRight /></Link> : <a className="button primary" href={content.href}>{content.action} <ArrowRight /></a>}
      <div className="workflow-alternatives">
        <span>Other tools remain available by choice:</span>
        {path !== "dimensionality_reduction" && <Link to={`/studies/${projectId}/dimension-reduction?modelId=${model.id}`}>screen dimensions</Link>}
        {path !== "surrogate" && <Link to={`/studies/${projectId}/surrogates?modelId=${model.id}`}>build a surrogate</Link>}
      </div>
    </section>
  );
}
...
export function Workspace() {
  ...
  const analysisGroups = useMemo(() => [
    {
      ...
      description: "Propagate the input uncertainty and characterize the model response.",
      analyses: directAnalyses.filter((analysis) => ["monte_carlo", "eda", "convergence"].includes(analysis.key)),
    },
    {
      title: "Sensitivity & reliability",
      description: "Rank influential inputs or estimate a clearly defined failure event.",
      analyses: directAnalyses.filter((analysis) => !["monte_carlo", "eda", "convergence"].includes(analysis.key)),
    },
  ], [directAnalyses]);
  ...
  return (
    <div className="page workspace-page">
      <ProjectNav projectId={activeProjectId} projectName={activeProject?.name} />
      ...
      <section className={`studio-card ${savedModel ? "validated-studio" : ""}`}>
        ...
        {savedModel && (
          <ModelUnderstandingPane model={savedModel} projectId={activeProjectId} />
        )}
      </section>
