  ...
  return (
    <aside className="understanding-pane" aria-label="Model Understanding">
      ...
      >
        ...
        <p className="ai-interpretation-notice">
          The rendered equation and narrative are AI-interpreted from the
    ...
    <div className="page workspace-page">
        ...
        <div className="studio-authoring">
          ...
          {mode === "source" ? (
            <>
              ...
              <details className="equation-authoring-note">
                <summary>How equation rendering works for Python models</summary>
                <p>
                  Validation first derives closed-form LaTeX from straight-line
                  Python expressions. Models with loops or numerical solvers
                  still receive an exact <code>y = f(x)</code> fallback. The
                  authenticated Model Understanding request then sends a bounded
                  copy of this model definition to the configured AI provider for
                  a clearly labelled LaTeX interpretation. You can also declare a
                  bounded <code>model_equations</code> list for procedural governing
                  equations.
                </p>
              </details>
            </>
      ...
      >
        ...
        >
          ...
          {selected.some((key) => key === "hsic" || key === "target_hsic") && (
            <section className="hsic-method-comparison" aria-label="HSIC method comparison">
              <header>
                <strong>Two complementary HSIC questions</strong>
                <span>Select either method independently, or run both on the same model.</span>
              </header>
              <div>
                <article className={selected.includes("hsic") ? "selected" : ""}>
                  <span>Whole response</span>
                  <strong>Global HSIC</strong>
                  <p>Ranks dependence between each input and the output across its full sampled range. No threshold is used.</p>
                </article>
                <article className={selected.includes("target_hsic") ? "selected" : ""}>
                  <span>Critical region</span>
                  <strong>Target-domain HSIC</strong>
                  <p>Ranks association with the output entering a defined region. It is not an event-probability estimate.</p>
                </article>
              </div>
            </section>
          )}
          {selected.includes("target_hsic") && (
            <div className="analysis-settings target-hsic-settings">
              <section className="target-hsic-studio">
                <header>
                  <div className="target-step-number">1</div>
                  <div>
                    <strong>Define the target region</strong>
                    <p>
                      Choose which side of a threshold is scientifically important for output{" "}
                      <code>{savedModel?.metadata.outputs[outputTarget]?.name ?? outputTarget}</code>.
                    </p>
                  </div>
                </header>
                <div className="target-domain-fields">
                  <label>
                    ...
                    >
                      <option value=">=">Output at or above threshold</option>
                      <option value="<=">Output at or below threshold</option>
                    </select>
                  </label>
                  <label>
                ...
                {savedModel?.assessment?.profile.pilot_outputs[outputTarget] &&
                  (() => {
                    ...
                    return (
                      <div className={`target-pilot-card ${intersects ? "intersects" : "outside"}`}>
                        <div className="target-step-number">2</div>
                        <div>
                          <strong>Check bounded pilot coverage</strong>
                          <p>
                            Observed {pilot.minimum.toPrecision(4)} to{" "}
                            {pilot.maximum.toPrecision(4)} · 5–95%{" "}
                            {pilot.quantile_05.toPrecision(4)} to{" "}
                            {pilot.quantile_95.toPrecision(4)}
                          </p>
                          <small>
                            {intersects
                              ? "The threshold intersects the validation pilot. Execution still requires at least five analysis samples on each side."
                              : "The threshold is outside the validation pilot. Adjust it before running or the target screen may reject insufficient coverage."}
                          </small>
                        </div>
                      </div>
                    );
                  })()}
                <div className="target-permutation-card">
                  <div className="target-step-number">3</div>
                  <div>
                    <strong>Set permutation evidence</strong>
                    <p>More replicates strengthen the independence diagnostic but increase quadratic kernel work.</p>
                  </div>
                  <label>
                    <span>Permutation replicates</span>
                    <input
                      type="number"
                      min="0"
                      max="200"
