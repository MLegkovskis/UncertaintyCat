...
def create_multivariate_distribution(distributions, variable_names=None):
    """
    Create a multivariate distribution from a list of marginal distributions.
    ...
    """
    # Create descriptions if variable names are provided
    if variable_names:
        for i, dist in enumerate(distributions):
            if i < len(variable_names):
                dist.setDescription([variable_names[i]])
    
    # Create independent multivariate distribution
    joint_dist = ot.JointDistribution(distributions)
                ...
                if st.session_state.selected_distributions:
                    ...
                    if create_button:
                        try:
                            ...
                            # Create joint distribution
                            joint_dist = create_multivariate_distribution(selected_dists, selected_variables)
                            
                            # Store in session state
                            st.session_state.problem_distribution = joint_dist
                            
                            st.success("Joint distribution created successfully! You can now use this distribution for uncertainty quantification analysis.")
...
# Sample function of interest (modify as needed for your analysis)
def function_of_interest(X):
    {', '.join([var.replace(' ', '_') for var in selected_variables if var in st.session_state.selected_distributions])} = X
    Y = {' + '.join([var.replace(' ', '_') for var in selected_variables if var in st.session_state.selected_distributions])}
    return [Y]

model = ot.PythonFunction({len([var for var in selected_variables if var in st.session_state.selected_distributions])}, 1, function_of_interest)
...
"""
                            ...
                            dist_code += ",\n    ".join([f"{var.replace(' ', '_')}" for var in selected_variables if var in st.session_state.selected_distributions])
                            dist_code += "\n])\n\n"
