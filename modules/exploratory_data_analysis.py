...
def exploratory_data_analysis(data, N, model, problem, model_code_str, language_model='groq'):
    ...
    fig_key = 'eda_fig'  # Key for the main figure
    clustermap_key = 'eda_clustermap_fig'  # Key for the clustermap figure
    data_key = 'eda_data'

    # Check if the results are already in session state
    if (response_key in st.session_state and 
        fig_key in st.session_state and 
        clustermap_key in st.session_state and 
        data_key in st.session_state):

        # Retrieve from session state
        response_markdown = st.session_state[response_key]
        fig = st.session_state[fig_key]
        clustermap_fig = st.session_state[clustermap_key] 
        ...
        existing_columns = list(variables) + ['Y']  # Convert variables to a list
        ...
        # --- Store figures in session_state ---
        st.session_state[fig_key] = fig
        st.session_state[clustermap_key] = clustermap_fig
